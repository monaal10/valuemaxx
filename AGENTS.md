...
### Continuous optimization application is explicitly bounded (ratchet, 2026-08-22)

An optimization recommendation remains evidence and never applies itself. Production
application is a separate, tenant-scoped deployment authorized for one call site. Its
ramp is exactly `1% -> 5% -> 25% -> 100%`, and automatic rollback may use only fast
signals (error, refusal, parse failure, and p95 latency), never delayed outcome rate.
Guardrail: `optimization_application_safety` checks the fixed ramp and ensures the
rollback vocabulary cannot acquire `outcome_rate`; core model validators require an
explicitly enabled per-call-site policy before a deployment can exist.
